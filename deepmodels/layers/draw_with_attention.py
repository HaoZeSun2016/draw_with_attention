...
class DrawLayer(Layer):
    ...
    def get_cost(self, x, y=None, *args, **kwargs):
        ...
        def _birnn_step(e, mask , ht_m_1, W , b ):
            """
            e: embedding sequences that indicate the index, note that the lenth is differ
                        e = f( dot(W_e,X) + b_e); X = W_w * label
                        shape=
            mask: indicate with position of the label_embd is actually empty
                        shape=(max_lenth) sliced from masks, row vector
            ht_m_1: previous hidden layer output
                        shape=           row vector
            W,b: parameters for BiRNN
                 for each direction: h_t = f( dot( ht_m_1 , W) + b+ label_embd)


            TO DO : test the code
            """
            tmp_ht = active_function( T.dot( ht_m_1 , W ) + e + b)
            return (1-mask) * ht_m_1 + mask * tmp_ht

        def birnn( label, masks):
            """
            label: label of images, which is already regularized to same lenth.
                   type: int matrix, shaoe=(num_batch , max_lenth)
            masks: {0,1} maxtrix indicates the empty positions
                    shape=(num_batch , max_lenth)
            self.birnn_output_mode: string "comcat" and "sum", W_d has different shape

            TO DO : test the code
            """
            l_m = T.zeros(shape=(self.num_batch,self.max_lenth , self.index_classes),dtype=theano.config.floatX)
            for i in range(self.index_classes):
                l_m[(label == i),i] = 1.0
            #
            x_w = T.dot(l_m , self.W_w)
            e = active_function( T.dot(x_w , self.W_e) + self.b_e)

            #
            ones = T.ones((self.num_batch,1))
            f_bih = theano.scan(_birnn_step(), sequences=[e,masks],
                                outputs_info=T.dot(ones,self.init_f_bih),
                                non_sequences=[self.W_f, self.b_h])
            b_bih = theano.scan(_birnn_step(), sequences=[e,masks],
                                outputs_info=T.dot(ones,self.init_f_bih),
                                non_sequences=[self.W_b, self.b_b],
                                go_backwards= True)
            #
            if self.birnn_output_mode == 'concat':
                hid_output = T.concatenate([f_bih ,b_bih] , axix = 1 )
            else:
                hid_output = f_bih + b_bih
            #
            return active_function( T,dot(hid_output , self.W_d) + self.b_d)
